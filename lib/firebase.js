...
export const ref = doc(db, 'tracks', 'deepak')

export async function __removeTagByName(tagName) {
    await getDoc(ref)
        .then(res => {
            const { tags } = res.data()
...
export async function __publishTag(tagData, callback) {
    await updateDoc(ref, { tags: arrayUnion(tagData) })
        .then(callback())
        .catch(err => console.log(err))
}

export async function __storeHourTrack(tagData, callback) {
